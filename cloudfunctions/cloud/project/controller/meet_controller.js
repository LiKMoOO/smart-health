...
class MeetController extends BaseController {
	...
	async getMeetList() {
		...
		// 确保list是一个有效的数组
		if (!Array.isArray(result.list)) result.list = [];
	...
	async viewMeet() {
		...
		try {
			...
			// 显示转换  
			if (meet.MEET_OBJ && meet.MEET_OBJ.desc) {
				meet.MEET_OBJ.desc = util.mdToHtml(meet.MEET_OBJ.desc);
			}
			
			return meet;
		} catch (err) {
			console.error('预约详情查询出错：', err);
			return null;
		}
	}
